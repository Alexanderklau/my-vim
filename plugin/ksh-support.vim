...
if has("autocmd")
	...
	if	s:MSWIN
		...
		"
		exe "autocmd BufNewFile,BufRead          *.ksh set fileformat=".s:BASH_FileFormat
	endif
	"
	autocmd BufNewFile,BufRead           *.ksh call BASH_CreateMenusDelayed()
	"
	" Bash-script : insert header, write file, make it executable
	"
	if !exists( 'g:BASH_AlsoBash' )
		"
		autocmd BufNewFile,BufRead           *.ksh set filetype=sh
		" style is taken from s:BASH_Style
		autocmd BufNewFile                   *.ksh call BASH_InsertTemplate("comment.file-description")
		autocmd BufRead                      *.ksh call BASH_HighlightJumpTargets()
		"
	else
		" 
